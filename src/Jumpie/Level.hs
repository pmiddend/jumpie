...
import Jumpie.Tuple(both,between)
...
validPlatforms ps' = validPlatforms' [] ps'
  where validPlatforms' :: [Platform] -> [Platform] -> [Platform]
        ...
        intersects ps p = False
        unreachable :: [Platform] -> Platform -> Bool
        --unreachable ps p = not $ or (pure (pReachable p) <*> ps)
        unreachable ps p = False
...
pParabolaIntersects p (LineSegment l r) = or $ parabolaPointIntersects <$> [playerParabola,paraInvert playerParabola] <*> [l - p,r - p]

playerParabola :: Parabola Real
playerParabola = Parabola (gcGrv/2,gcJmp,0)
{-
...
pParabolaIntersects :: PointReal -> LineSegmentReal -> Bool
pParabolaIntersects pstart (LineSegment (Point2 lx1 y) (Point2 lx2 _)) = xl `between` (lx1,lx2) || xr `between` (lx1,lx2)
